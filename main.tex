...
\begin{figure}[t]
\centering
\includegraphics[width=0.88\linewidth]{figures/memaudit_write_time_evaluation.png}
...
\Cref{tab:reliability} is intentionally conservative. The primary Natural-200 export used 681 API calls, 1{,}978{,}327 tokens, and about \$0.907. An initial secondary audit surfaced ambiguous required-unit cases, motivating a stricter Gemini-Flash adjudication pass on 100 examples; it accepted or corrected 87 examples and flagged 13 ambiguous examples. The independent 30-example spot-check accepted/corrected 29 examples. This is an advantage of the package format: ambiguous evidence requirements become measurable dataset objects rather than hidden benchmark noise.
...
\method's scores are package-conditional by design: the finite candidate table, cost model, evidence schema, and budget define the evaluated object. This does not replace end-to-end assistant evaluation, but it supplies the missing write-time diagnostic. If a generator omits the right memory, the package optimum measures the best store within that table; if a writer extracts useful memories but fails to retain the right subset, union and upper-pruned scores expose the selection bottleneck. The cost model and one-representation constraint are explicit package parameters; the released sensitivity audit recomputes Natural-87 under a byte-overhead cost and a $k=2$ representation relaxation. The $k=1$ default is a benchmark choice, not a claim that deployed systems must store one representation per experience; allowing $k=2$ changes exact OPT by only $1.3$--$6.0\%$ on Natural-87, so the constraint affects ratios at the percentage-points level rather than changing the qualitative rankings.

The natural support-sliced package is a conservative first natural instantiation. Model-adjudicated labels provide a scalable construction method with explicit rejection, correction, and audit hooks, and the released human coverage audit shows that the resulting coverage rows are stable under independent human review on the adjudicated subset. Support slices isolate the write-time problem before full-haystack expansion introduces retrieval and session-selection confounds.

The oracle reference writer uses exact coverage marginals, so deployed systems will need estimated utilities, learned candidate generators, model-based annotators, compaction, or native budget-aware admission. \method\ makes those extensions measurable by reporting package ratios, union ratios, and downstream QA metrics side by side. Exact optimization currently targets small and medium packages; larger packages can use decomposition, MILP certificates with gaps, or exact evaluation on sampled slices. The LongMemEval-S transfer study is therefore included as a retrieval/reader diagnostic, not as the primary denominator claim.

The artifact contains exact and stress generators, branch-and-bound and MILP solvers, natural exports, adjudication summaries, public-system scoring scripts, cached API metadata, and figure-generation scripts. Minimal reproduction does not require API calls; API-backed construction is needed only to regenerate natural annotations or rerun public-system exports. We recommend reporting the package schema, cost rule, solver status, oracle ratio, union ratio, and downstream QA metric as separate columns, so write quality, extraction, selection, retrieval, and reader behavior remain auditable.
